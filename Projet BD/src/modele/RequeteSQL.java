...
public class RequeteSQL {
	...
	public static int getEmprunt(Connection connexion,String id_livre) throws SQLException {
		Statement stmt = connexion.createStatement();
		String str = "SELECT COUNT(*) FROM EXEMPLAIRE,EMPRUNT WHERE EMPRUNT.ID_EX=EXEMPLAIRE.ID_EX AND DATE_RETOUR IS NULL AND ID_LIVRE = '"+id_livre+"'";
		ResultSet rset = stmt.executeQuery(str);
		int res = 0;
		while(rset.next()) {
				res += rset.getInt("COUNT(*)");
	...
	public static void setEmprunt(Connection connexion, String id_et,String id_ex) throws SQLException {
		...
		String str = "INSERT INTO EMPRUNT (ID_ET,ID_EX,DATE_EMP) VALUES ("+id_et+","+id_ex+",TO_DATE('"+date_emp+"','yyyy-mm-dd hh:mi:ss'))" ;
	...
	public static Retard[] getRetard(Connection connexion) throws SQLException {
        Date date = Calendar.getInstance().getTime();
        DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
        String date_empf = dateFormat.format(date);
        Retard[] res = new Retard[getNumberOfRows2(connexion,"SELECT titre,date_emp,nom,prenom,email FROM EMPRUNT,LIVRE,ETUDIANT,EXEMPLAIRE WHERE LIVRE.ID_LIVRE = EXEMPLAIRE.ID_LIVRE AND EXEMPLAIRE.ID_EX = EMPRUNT.ID_EX AND EMPRUNT.ID_ET = ETUDIANT.ID_ET AND DATE_RETOUR IS NULL AND DATE_EMP - TO_DATE('"+date_empf+"','yyyy-mm-dd hh:mi:ss') >15")];
        Statement stmt = connexion.createStatement();
        ResultSet rset = stmt.executeQuery("SELECT titre,date_emp,nom,prenom,email FROM EMPRUNT,LIVRE,ETUDIANT,EXEMPLAIRE WHERE LIVRE.ID_LIVRE = EXEMPLAIRE.ID_LIVRE AND EXEMPLAIRE.ID_EX = EMPRUNT.ID_EX AND EMPRUNT.ID_ET = ETUDIANT.ID_ET AND DATE_RETOUR IS NULL AND DATE_EMP - TO_DATE('"+date_empf+"','yyyy-mm-dd hh:mi:ss') >15");
        int index =0;
        while(rset.next()) {

            //System.out.println("test");
            String titre = rset.getString("titre");
                String date_emp = rset.getString("date_emp");
                String nom = rset.getString("nom");
                String prenom = rset.getString("prenom");
                String email = rset.getString("email");
                //System.out.println("id: "+id_et+" nom: "+nom+" prenom: "+prenom+" email: "+email+" mdp: "+mdp);

            res[index] = new Retard(titre,date_emp,nom,prenom,email);
            index++;
        }
        stmt.close();
    return res;
